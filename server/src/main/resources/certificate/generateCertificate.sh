...
openssl x509 -req -days 360 -in client/request.csr -CA server/server.crt -CAkey server/serverPrivateKey.pem -CAcreateserial -out client/sikb.crt -sha256
## Client
# Verify client's certificate
openssl x509 -text -noout -in client/sikb.crt
# Create PKCS12 keystore containing client's private key and related self-sign certificate 
openssl pkcs12 -export -out client/client_pavel.p12 -inkey client/myPrivateKey.pem -in client/sikb.crt -certfile server/server.crt
